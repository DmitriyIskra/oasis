...
Вопрос Андрею:
При повторной отправке кода, что на сервер отправить? или может какая то сессия откроется и сервер по кукам сам поймет что за пользователь
Нормально ли от проверки кода возвращать назад, ведь там пользователь может корректировать. и Опять произойдет отправка
Когда лучше отправлять данные после второй попап или после проверки кода
